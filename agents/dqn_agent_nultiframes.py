...
class DQNAgent:
    ...
    def select_action(self, state):
        ...
        if sample > eps_threshold:
            ...
            with torch.no_grad():
                ...
                actions = action_list.max(1)
                max_prob = torch.max(actions[0], 0)[1]
                action = actions[1][max_prob].view(1, 1)
                # if 8 < action < 13:
    ...
    def optimize_model(self):
        ...
        # concatenate 4 frames states
        state_batch = torch.cat(tuple(state for state in batch.state))
        next_state_batch = torch.cat(tuple(state for state in batch.next_state))
        # print(state_batch.shape, next_state_batch.shape)
